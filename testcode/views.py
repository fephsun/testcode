...
initial_testing_script = '''cases = [('in1', 'out1'), ('in2', 'out2')]
def case_tester(fn, cases):
    score = 0
    for in_value, expected_out in cases:
        actual_out = fn(in_value)
        if actual_out == expected_out:
            print "Correct output for " + str(in_value)
            score += 1
        else:
            print "Incorrect output for " + str(in_value)
            print "Expected " + str(expected_out)
            print "Got " + str(actual_out)
    print score
case_tester(my_fn, cases)
'''
...
def student(request):
    ...
    try:
        ...
        return HttpResponseRedirect('/testcode')
    currentUser = ""
    isNewUser = False
    courses = []
...
def getlecturestudent(request):
    ...
    if ("course_id" in request.POST):
        ...
        for count in range(len(lectures)):
            ...
            lecture_name.append(thisLecture.description)
            lecture_id.append(thisLecture.lecture_id)
            myproblems.append(len(problems)) 
        JsonDict["lecture_name"] = lecture_name
        JsonDict["lecture_id"] = lecture_id
        JsonDict["password"] = course.student_password
        JsonDict["problems"] = myproblems
    else:
        isOkay = False
        error = post_request_err
    JsonDict["isOkay"] = isOkay
...
def edit(request, mylecture_id):
    ...
    enrollment = Enrollment.objects.filter(user=currentUser, course=course)[0]
    problems = Problem.objects.filter(lecture=lecture).order_by('problem_number')
...
def createproblem(request):
    ...
    if ("lecture_id" in request.POST) and ("problem_name" in request.POST):
        ...
        if len(problem_name) > 0:
            ...
            newProblemNumber = 0
            #TODO: This is an inefficient way to find the latest problem number, I think.
            ...
            newProblem = Problem(name=problem_name, lecture=lecture, 
                                 problem_number=newProblemNumber, description="", 
                                 initial_code="", testing_script=initial_testing_script)
            newProblem.save()
    ...
    # Case 2 - problem_id, description, initial_code, timeout
        ...
        testing_script = request.POST["testing_script"]
        soln = request.POST["soln"]
        if len(description) > 0:
            ...
            problem.testing_script = testing_script
            problem.soln = soln
            problem.save()
            
            if soln != '':
                # Test-grade the solution.
                results, score, error = test(soln, testing_script)
    ...
    # Case 3 - None of the above.  Return error.
    else:
        isOkay = False
        error = post_request_err
    JsonDict["isOkay"] = isOkay
...
def getproblem(request):
    ...
    if ("problem_id" in request.POST):
        ...
        currentEnrollment =  Enrollment.objects.filter(user=currentUser, course=problem.lecture.course)[0]
...
def teacherlecture(request, lecture_id):
    ...
    allProblems = Problem.objects.filter(lecture=lecture_id).order_by('problem_number')
    problems = []
...
def getproblemteacher(request):
    ...
    if ("problem_id" in request.POST):
        ...
        testing_script = problem.testing_script
        JsonDict["problem_description"] = problem_description
        JsonDict["name"] = problem.name
        JsonDict["initial_code"] = problem.initial_code
        JsonDict["testing_script"] = testing_script
        JsonDict["soln"] = problem.soln
    else:
        isOkay = False
        error = post_request_err
    JsonDict["isOkay"] = isOkay
...
def deleteproblem(request):
    """
    Deletes a problem
    - Chosen by 'problem_id' key in POST 
    """
    try:
        user_id = request.session["user_id"]
        currentUser = User.objects.get(user_id=user_id)
        ...
        return HttpResponseRedirect('/testcode')
    id_to_delete = request.POST['problem_id']
    course = request.POST['course']
    problem = Problem.objects.get(problem_id=id_to_delete)
    problem.delete()
    json = simplejson.dumps({'isOkay': True})
    return HttpResponse(json, content_type="application/json")

def reorderproblems(request):
    """
    Takes in a list of problem_id's, and changes their problem_number's
    to reorder them in the specified order.
    POST: 'id_list'
    """
    try:
        user_id = request.session["user_id"]
    ...
    for i in xrange(len(id_list)):
        problem = Problem.objects.get(problem_id=id_list[i])
...
def saveandrun(request):
    ...
    if ("problem_id" in request.POST) and ("solution" in request.POST):    
        ...
        problem = Problem.objects.get(problem_id=problem_id)
        if len(solution) > 0:
            enrollment = Enrollment.objects.filter(course=problem.lecture.course, user=currentUser)[0]
            ...
            #newSubmission.save()
            result = test(solution, problem.testing_script)
            newSubmission.grade=result[1] #returns JSON with results of all testcases
            print "Saving..."
            newSubmission.save()
            print "making json dicts"
            JsonDict["grade"] = result[1]
            JsonDict["feedback"] = result[0]
            JsonDict["errors"] = result[2]
        else:
            isOkay = False
...
def home(request):
    ...
    try:
        ...
        return HttpResponseRedirect('/testcode')
    if currentUser.isAdmin:
        return HttpResponseRedirect('/testcode/teacher')
    else:
        return HttpResponseRedirect('/testcode/student')
    ...
    if ("problem_id" in request.POST):
        ...
        for enrollment in enrollments:
            ...
            if len(submissions) > 0:
                ...
                # subgrade = []
                # for thing in submission.grade:
                #     subgrade.append(thing)
...
def test(submission, test_code):
    '''
    Tests submission against test_code, both of which are strings.
    '''
    total_code = submission + '\n' + test_code
    # from ideone.ideone import *
    # ideone = IdeOne()
    # python = 4
    # link = ideone.createSubmission(total_code, python)
    ...
    # # Get the score, which must be on the last line.
    # lines = output.splitlines()
    # score = int(lines[-1])
    # return (output, score, error)

    import sandbox
    import sys
    from cStringIO import StringIO
    myBox = sandbox.Sandbox(sandbox.SandboxConfig('stdout', 'itertools', 'math', 'random'))
    error = ''
    out = ''
    score = 0
...
def studentperformance(request, problem_id):
    ...
    submissions = 0
    for enrollment in enrollments:
        if len(Submission.objects.filter(problem=problem, enrollment=enrollment)) > 0:
            submissions += 1
    testcases = []
    t = get_template("teacher-analyze.html")
    rc = Context({"user": currentUser, 
                  "course": course, 
                  "lecture": lecture, 
                  "problem": problem, 
                  "testcases": testcases, 
